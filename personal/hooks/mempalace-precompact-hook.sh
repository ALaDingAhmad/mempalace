...
#
# Personalized fork of hooks/mempal_precompact_hook.sh.
# Difference vs upstream: replaces bare `mempalace` calls with
# `"$MEMPAL_PYTHON_BIN" -m mempalace` so it works on systems where the
# `mempalace` CLI script is not on PATH (common on Windows Store Python,
# pyenv-shim'd setups, etc).
#
# Install:
#   cp personal/hooks/mempalace-precompact-hook.sh ~/.claude/
#   chmod +x ~/.claude/mempalace-precompact-hook.sh
#   # Then add PreCompact entry to ~/.claude/settings.json (see claude-settings.json.example)
#
